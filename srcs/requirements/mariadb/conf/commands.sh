mysql -e "CREATE DATABASE IF NOT EXISTS MYSQL_DATABASE;"

mysql -e "CREATE USER IF NOT EXISTS 'MYSQL_USER'@'localhost';"
mysql -e "SET PASSWORD FOR 'MYSQL_USER'@'localhost' = PASSWORD('MYSQL_PASSWORD');"
mysql -e "GRANT ALL PRIVILEGES ON MYSQL_DATABASE.* TO 'MYSQL_USER'@'localhost' IDENTIFIED BY 'MYSQL_PASSWORD';"
mysql -e "FLUSH PRIVILEGES;"

mysql -e "CREATE USER IF NOT EXISTS 'MYSQL_SUPERUSER'@'localhost' IDENTIFIED BY 'MYSQL_SUPERPASSWORD';"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'MYSQL_SUPERUSER'@'localhost' WITH GRANT OPTION;"
mysql -e "FLUSH PRIVILEGES;"

mysql -e "ALTER USER 'root'@'localhost' IDENTIFIED BY 'MYSQL_ROOT_PASSWORD';"
